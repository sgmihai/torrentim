...
struct(*udpTrackerConnectPing, endian = b):
...
struct(*udpTrackerScrapePing, size: uint32, endian = b):
  64: *connection_id = connection_id
  32: *action = 2
  32: *transaction_id = rand(int32)
  s: *info_hashes(20)[size]
...
struct(*udpTrackerScrapePong, size: int, endian = b):
  32: *action
  32: *transaction_id
  *udpTrackerScrapePongInfo: *info{s.atEnd}
...
struct(*udpTrackerAnnouncePongPeerList, endian = b):
